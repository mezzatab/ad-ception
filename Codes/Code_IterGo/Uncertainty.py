...
class Uncertainty:
    ...
    def __init__(self, df, sampleSize, forecastWindow, shortTerm, nPredict, nSections=10, engine='2layerRNN'):
        self.Preds=[]
        (self.tTime,xAxis)=df.shape
        self.nPredict=nPredict
        self.sampleSize=sampleSize
        self.fWindow=forecastWindow
...
#        self.nSections=1+int((self.tTime-self.sampleSize)/(self.testSize))
        for i in range(nPredict):
            self.Preds.append(WellHeadIterativeGo.WellheadPressurePrediction(df, self.sampleSize, self.fWindow, self.shortTerm, self.engine))
    ...
    def Run(self, WPP): 
        ...
        self.meano=self.df[:t0,4].mean()
        for i in range(self.nSections):
            print(i)
            t1=time.time()
            WPP.train(t0+i*self.fWindow)
            t2=time.time()
            dt=t2-t1
            ...
            WPP.predict()   
        filter_size=59            
        WPP.cYp_test = medfilt(WPP.cYp_test,filter_size)        
...
#WPP.cYp_test             
    ...
    def uncertaintyAnalysis(self):
        ...
        for i in range(self.nSections-1):
            self.metric(self.cY[i*self.fWindow:(i+1)*self.fWindow],self.avgCalc[i*self.fWindow:(i+1)*self.fWindow])
            self.bestMetric(self.cY[i*self.fWindow:(i+1)*self.fWindow],self.Preds[self.bestPred].cYp_test[i*self.fWindow:(i+1)*self.fWindow])
            self.worstMetric(self.cY[i*self.fWindow:(i+1)*self.fWindow],self.Preds[self.worstPred].cYp_test[i*self.fWindow:(i+1)*self.fWindow])
            self.updateMean(self.cY[i*self.fWindow:(i+1)*self.fWindow])            
        i=self.nSections-1
        self.metric(self.cY[i*self.fWindow:i*self.fWindow+self.Preds[0].rT],self.avgCalc[i*self.fWindow:i*self.fWindow+self.Preds[0].rT])
        self.bestMetric(self.cY[i*self.fWindow:i*self.fWindow+self.Preds[0].rT],self.Preds[self.bestPred].cYp_test[i*self.fWindow:i*self.fWindow+self.Preds[self.bestPred].rT])
        self.worstMetric(self.cY[i*self.fWindow:i*self.fWindow+self.Preds[0].rT],self.Preds[self.worstPred].cYp_test[i*self.fWindow:i*self.fWindow+self.Preds[self.worstPred].rT])
    ...
    def bestPredictor(self):
        ...
        self.bestPred=0
        self.worstPred=0
        rmBest=self.METRICS['RMSE'](self.cY[:i*self.fWindow+self.Preds[0].rT],self.Preds[0].cYp_test[:i*self.fWindow+self.Preds[0].rT])
        rmWorst=rmBest
        for j in range(self.nPredict):
            rm=self.METRICS['RMSE'](self.cY[:i*self.fWindow+self.Preds[j].rT],self.Preds[j].cYp_test[:i*self.fWindow+self.Preds[j].rT])
            if rm < rmBest:
                self.bestPred=j
                rmBest=rm
            if rm > rmWorst:
...
#        self.metric(self.cY[i*self.testSize:i*self.testSize+self.Preds[0].rT],self.avgCalc[i*self.testSize:i*self.testSize+self.Preds[0].rT])
    ...
    def NMSE(self, Yo, Yp):
        ...
        for i in range(len(Yo)):
            ...
            denumerator=denumerator+(Yo[i]-self.meano)**2
#        return 1-numerator/denumerator
        return 1-numerator/denumerator

    def updateMean(self, Yo):  
        (L,)=Yo.shape         
        sum_o=0
        for i in range(L):
            sum_o+=Yo[i]
    ...
    def NRMSE(self, Yo, Yp):
        ...
        for i in range(len(Yo)):
            ...
            denumerator=denumerator+abs(Yo[i]-self.meano)
        return 1-numerator/denumerator    
    
    def rmean_squared_error(self, Yo, Yp):
        Sum=0
        (L,)=Yp.shape        
#        print(Yo.shape,Yp.shape)
        for i in range(L):
            Sum=Sum+(Yo[i]-Yp[i])**2
    ...
    def writeOutPut(self,file):
        ...
        self.Metric={'Section':[self.Preds[0].cTime[self.fWindow*i] for i in range(self.nSections)], 'NMSE':self.metricsGrowth['NMSE'], 'NRMSE':self.metricsGrowth['NRMSE'],'MAPE':self.metricsGrowth['MAPE'], 'SMAPE':self.metricsGrowth['SMAPE'],'RMSE':self.metricsGrowth['RMSE'],'CalcTime':self.compTime}        
        self.BestMetric={'Section':[self.Preds[self.bestPred].cTime[self.fWindow*i] for i in range(self.nSections)], 'Best_NMSE':self.bestMetricsGrowth['NMSE'], 'Best_NRMSE':self.bestMetricsGrowth['NRMSE'],'Best_MAPE':self.bestMetricsGrowth['MAPE'], 'Best_SMAPE':self.bestMetricsGrowth['SMAPE'],'Best_RMSE':self.bestMetricsGrowth['RMSE']}        
        self.WorstMetric={'Section':[self.Preds[self.worstPred].cTime[self.fWindow*i] for i in range(self.nSections)], 'Worst_NMSE':self.worstMetricsGrowth['NMSE'], 'Worst_NRMSE':self.worstMetricsGrowth['NRMSE'],'Worst_MAPE':self.worstMetricsGrowth['MAPE'], 'Worst_SMAPE':self.worstMetricsGrowth['SMAPE'],'Worst_RMSE':self.worstMetricsGrowth['RMSE']}                
