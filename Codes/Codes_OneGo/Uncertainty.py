...
#filter_size = 59
import WellHeadVal #import Class of Wellhead for IterGo Approach ## 
#import WellHeadVal 
#np.random.seed(42)
#set_random_seed(42)
...
class Uncertainty:
    ...
    def __init__(self, df, sampleSize, testSize, shortTerm, nPredict, nSections=10, engine='2layerRNN'):
        self.Preds=[]
...
#        self.nSections=1+int((self.tTime-self.sampleSize)/(self.testSize))
        for i in range(nPredict):
            self.Preds.append(WellHeadVal.WellheadPressurePrediction(df, self.sampleSize, self.testSize, self.shortTerm, self.engine))
...
#        self.metric(self.cY[i*self.testSize:i*self.testSize+self.Preds[0].rT],self.avgCalc[i*self.testSize:i*self.testSize+self.Preds[0].rT])
    ...
    ####################################    ####################################     ####################################
            ...
            #### Writing out the final results, forecasts, sigma, avg, and Computational time and performance on each section #####            
